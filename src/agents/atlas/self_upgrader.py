...
def run_upgrade(shared_state: dict = None, use_llm: bool = True) -> dict:
    ...
    from src.agents.atlas.core import _get_conn, log_synthesis_run
    from src.agents.atlas.options_flow_memory import rebuild_insights
    from src.agents.atlas.news_impact_mapper import build_impact_map
    from src.agents.atlas.regime_detector import build_time_pattern_stats
    from src.agents.atlas.volume_classifier import build_volume_stats
    from src.agents.atlas.causal_engine import (
...
def _generate_rules_from_patterns(conn) -> tuple:
    ...
    """
    from src.agents.atlas.core import store_rule, get_active_rules
...
def run_quick_context_refresh(shared_state: dict) -> dict:
    ...
    """
    try:
        from src.agents.atlas.core import get_knowledge_stats, get_best_patterns, get_active_rules
        from src.agents.atlas.regime_detector import detect_regime, get_time_context, get_time_win_rate
        from src.agents.atlas.options_flow_memory import get_options_context
